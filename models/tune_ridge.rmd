...
tune_ridge_h1 <- function(df) {
  ...
  cat(sprintf("λ* = %.6g | CV MSE = %.6g (over %d folds)\n",
...
tune_ridge_h3 <- function(df) {
  ...
  # Remove last 3 rows (where Y is NA for t+3) for consistent training/test
  valid_rows <- !is.na(Y)
  X_clean <- X[valid_rows, , drop = FALSE]
  Y_clean <- Y[valid_rows]
...
tune_ridge_h6 <- function(df) {
  ...
  # Remove last 6 rows (where Y is NA for t+6) for consistent training/test
  valid_rows <- !is.na(Y)
  X_clean <- X[valid_rows, , drop = FALSE]
  Y_clean <- Y[valid_rows]
  ...
  rows_count      <- nrow(X_clean)          # should be 222
  validation_size <- rows_count - min_train_size # should be 100
...
tune_ridge_h12 <- function(df) {
  ...
  # Remove last 12 rows (where Y is NA for t+12) for consistent training/test
  valid_rows <- !is.na(Y)
  X_clean <- X[valid_rows, , drop = FALSE]
  Y_clean <- Y[valid_rows]
