...
Structure your results here. The placeholders below can be replaced with your code snippets or embedded tables/figures from `results/` and `plots/`.

### 1) Data & Preprocessing Summary

    # [R CODE 1: DATA_SUMMARY]
    # e.g., read predictors, apply publication-lag alignment, show sample period and N

    # [PY CODE 1: DATA_SUMMARY]
    # Optional Python version of the same summary

### 2) Statistical Accuracy

- Out-of-sample performance by horizon (R², RMSE).

    # [R CODE 2: BUILD_STAT_ACCURACY_TABLE]
    # Produces a table saved to results/stat_accuracy.md (or .tex)

    <!-- [TABLE INCLUDE] paste the rendered table markdown below or link to the file -->
    <!-- e.g., -->
    <!-- ![Statistical accuracy](plots/stat_accuracy.png) -->

### 3) Diebold–Mariano Tests

    # [R CODE 3: DM_TESTS]
    # Computes DM statistics and p-values comparing candidate models vs Prevailing Mean (and Lasso v1 vs v2)

    <!-- [TABLE INCLUDE] paste the DM results table or image here -->

### 4) Forecast Plots

    # [R CODE 4: PLOTS]
    # Generates figures: ERP over time, forecast vs actual, residual diagnostics; save into plots/

    <!-- [FIGURE INCLUDE] embed key figures -->
    <!-- ![ERP over time](plots/erp_over_time.png) -->
    <!-- ![Forecast vs Actual (h=1)](plots/forecast_vs_actual_h1.png) -->

### 5) Economic Value / Portfolio Tests

- Dynamic allocation vs 60/40; turnover & cost assumptions.

    # [R CODE 5: ECONOMIC_VALUE]
    # Computes utility gains, Sharpe, drawdown, ES improvements, etc.; saves tables/figures

    <!-- [TABLE/FIGURE INCLUDE] paste economic value table(s) or embed plots from plots/ -->

### 6) Reproducibility Notes

- Required R packages: `glmnet`, `pls`, `randomForest`, `forecast`, `sandwich`, `xts`, etc.  
- Optional Python: `pandas`, `numpy`, `matplotlib`, `statsmodels`, `arch`.  
- Place generated artifacts under `results/` and `plots/` and reference them above.
