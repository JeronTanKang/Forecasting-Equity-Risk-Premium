...
THREE THIINGS TO DO BEFORE U CLICK RUN ALL
1. make sure full_df.csv is in ../data/ and u have the bond data file too
2. replace model code with ur own code, keep function name the same
3. choose ur h horizon at line 35
...
# model_name <- 'lasso_linear'
# model_name <- 'lasso_non_linear'
# model_name <- 'pls'
...
lasso_non_linear <- function(df, h){
  df_squared <- df %>%
    mutate(across(
      .cols = -date,
      .fns = ~ .x^2,
...
tree <- function(df,h){
  ...
  # standardize all predictors except date and erp
  df_std <- df %>%
  ...
  # holdout row for forecast = most recent row
  df_forecast <- df_std %>%
    slice(n()-h+1) %>%
    #select(MA_spread_1_9,	MA_spread_1_12,	MA_spread_2_9,	MA_spread_2_12,	MA_spread_3_9,	MA_spread_3_12,	OBV_spread_1_9,	OBV_spread_1_12,	OBV_spread_2_9,	OBV_spread_2_12,	OBV_spread_3_9,	OBV_spread_3_12)
    select(MA_sig_1_9,	MA_sig_1_12,	MA_sig_2_9,	MA_sig_2_12,	MA_sig_3_9,	MA_sig_3_12,	MOM_sig_9,	MOM_sig_12,	OBV_sig_1_9,	OBV_sig_1_12,	OBV_sig_2_9,	OBV_sig_2_12,	OBV_sig_3_9,	OBV_sig_3_12, skvw_lag2) # this is good, adding lag 1 is bad
    #select(MA_sig_1_9,	MA_sig_2_12,	MOM_sig_9,	OBV_sig_1_9,	OBV_sig_1_12,	OBV_sig_2_9,	OBV_sig_2_12)
    #select(MA_sig_1_9,	MA_spread_1_9,	MA_sig_1_12,	MA_spread_1_12,	MA_sig_2_9,	MA_spread_2_9,	MA_sig_2_12,	MA_spread_2_12,	MA_sig_3_9,	MA_spread_3_9,	MA_sig_3_12,	MA_spread_3_12,	MOM_sig_9,	MOM_sig_12,	OBV_sig_1_9,	#OBV_spread_1_9,	OBV_sig_1_12,	OBV_spread_1_12,	OBV_sig_2_9,	OBV_spread_2_9,	OBV_sig_2_12,	OBV_spread_2_12,	OBV_sig_3_9,	OBV_spread_3_9,	OBV_sig_3_12,	OBV_spread_3_12)

  # training data = rest of history
  df_train <- df_std %>% slice(1:(n() - h))

  # select target and predictors for training
  df_tree <- df_train %>%
    select(target, MA_sig_1_9,	MA_sig_1_12,	MA_sig_2_9,	MA_sig_2_12,	MA_sig_3_9,	MA_sig_3_12,	MOM_sig_9,	MOM_sig_12,	OBV_sig_1_9,	OBV_sig_1_12,	OBV_sig_2_9,	OBV_sig_2_12,	OBV_sig_3_9,	OBV_sig_3_12, skvw_lag2)
  # fit regression tree
  tree_fit <- rpart(
    formula = target ~ .,
    data = df_tree,
    ...
    control = rpart.control(
      cp = 0.0001,            # lower cp gives deeper tree
      minsplit = 10,          # need at least 5 rows to try a split
      xval = 10,              # 10-fold CV for cp pruning stats
      maxdepth = 15
    )
  )

  # 1-step forecast from the tree
  pred <- predict(tree_fit, newdata = df_forecast)
...
```{r}
## helper function to transform train predictors to RFF space ###
# build_rff_features <- function(X, num_features, gamma, seed) {
#   stopifnot(num_features %% 2 == 0)
#   set.seed(seed)
# 
...
# }
# 
# ### final forecasting model with baked-in tuned params ###
# model <- function(df, h) {
#   # 1. tuned hyper params
#   params <- switch(as.character(h),
#     "1"  = list(P=2000L, z=0.1,  gamma=0.2),
...
```{r}
.ALLOWED_MODELS <- c(
  'lasso_linear','lasso_non_linear','pls','ridgeless','ridge',
...
run_recursive_window <- function(df, model_name) {
  ...
  preds_df <- data.frame(
    date = date_col,                 
    model_name = rep(NA_real_, WINDOW_SIZE)
  )
...
forecast_eval <- function(df, model_name) {
  ...
  Y_h1_actual <- head(tail(lead(df$erp, 1), WINDOW_SIZE+1), WINDOW_SIZE) #these erp values are for calculating residual/comparing on plots
  Y_h1_predicted <- head(preds_df$model_name, WINDOW_SIZE)
  ...
  plot_df <- data.frame(
    date = tail(df$date, WINDOW_SIZE),  # Assuming the date column is the last column in 'df'
...
ggplot() +
  ...
  scale_color_manual(
    ...
    labels = c("const6040_ret" = "60/40 return",
               "timed_ret"     = "Timed return",
               "model_pred" = "Forecast Signal")  # Add label for the dashed line
  ) +
  scale_y_continuous(
    name = "Monthly Portfolio Return (%)",  # Left Y-axis label
    sec.axis = sec_axis(
      ~ . / scale_factor,
      name = "Forecast Signal (%)",  # Right Y-axis label 
    )
  ) +
  labs(
    x = "",
    color = NULL,
    title = "Monthly Portfolio Return (%): Market Timing vs Constant Portfolio and Forecast Signal"
  ) +
...
scale_factor_cum <- sd(plot_df_cum$cum_timed, na.rm = TRUE) /
                    sd(plot_df_cum$model_pred, na.rm = TRUE)
...
ggplot() +
  ...
  scale_color_manual(
    ...
    labels = c("cum_const6040" = "60/40 Cumulative Return", 
               "cum_timed" = "Timed Cumulative Return", 
...
ggplot(weight_df, aes(x = date, y = weight_sp)) +
  ...
  labs(
    title = "S&P500 Allocation Over Test Period (%)",
    x = "",
    y = ""
  ) +
  theme_minimal(base_size = 12) +
